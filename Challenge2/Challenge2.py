...
def BR(k, X_train, Y_train, n):
    ...
    for i in range(k):
        ...
        bagr = BaggingRegressor(n_estimators=n) #定义BaggingRegressor类对象，基本估计器数量参数最终选择300
        bagr.fit(data[0],data[1]) #训练集进行拟合
        Y_prediction = bagr.predict(data[2])#对验证集预测
        rmse_sum += rmse(data[3],Y_prediction)
        mape_sum += mape(data[3],Y_prediction)
    return rmse_sum / k, mape_sum / k
for i in range(1, 8):# 尝试不同的n_estimators
    result6 = BR(5, Xtrain, Ytrain, 50*i)
    rmse_error_synt['Bagging'] = result6[0]
    mape_error_synt['Bagging']  = result6[1]
    print("n_estimators={} \nMoyenne de RMSE={} \nMoyenne de MAPE={}".format(i*50, result6[0], result6[1]))
...
mse_scorer = make_scorer(mean_squared_error) #定义mse为标准的打分器
max_depth_CV = treereg.get_depth() #获取最大的深度
...
def RF(k, X_train, Y_train, n):
    ...
    return rmse_sum / k, mape_sum / k
for i in range(1, 8):# 尝试不同的n_estimators
    result8 = RF(5, Xtrain, Ytrain, 50*i)
    rmse_error_synt['RF'] = result6[0]
    mape_error_synt['RF']  = result6[1]
    print("n_estimators={} \nMoyenne de RMSE={} \nMoyenne de MAPE={}".format(i*50, result8[0], result8[1]))
...
# Boosting 选用梯度提升回归GradientBoostingRegressor
# 同上，先选择最优深度
...
mape_error_synt['L2Boosting']  = result9[1]
print("Moyenne de RMSE={} \nMoyenne de MAPE={}".format(result9[0],result9[1]))
result9 = L2Boosting(5, Xtrain, Ytrain, 100, 10, 0.3, 0.1)
rmse_error_synt['L2Boosting'] = result9[0]
mape_error_synt['L2Boosting']  = result9[1]
print("Moyenne de RMSE={} \nMoyenne de MAPE={}".format(result9[0],result9[1]))
result9 = L2Boosting(5, Xtrain, Ytrain, 200, 10, 0.3, 0.1)
rmse_error_synt['L2Boosting'] = result9[0]
mape_error_synt['L2Boosting']  = result9[1]
print("Moyenne de RMSE={} \nMoyenne de MAPE={}".format(result9[0],result9[1]))
result9 = L2Boosting(5, Xtrain, Ytrain, 300, 10, 0.3, 0.1)
rmse_error_synt['L2Boosting'] = result9[0]
mape_error_synt['L2Boosting']  = result9[1]
print("Moyenne de RMSE={} \nMoyenne de MAPE={}".format(result9[0],result9[1]))
...
def SVM(k, X_train, Y_train):
    ...
    for i in range(k):
        data = get_k_blocs_data(k, i, X_train, Y_train)
        boston_svr = SVR(kernel='rbf', gamma='auto', C=100000, epsilon = 0.00001) # 核函数选择RBF函数，gamma自动选取
...
def nn_train(net,train_features, train_labels, test_features, test_labels,
    ...
    train_iter = d2l.load_array((train_features, train_labels.unsqueeze(-1)),batch_size)# 包含训练集的特征和标签以及批尺寸（单次处理的样本个数）
    optimizer = torch.optim.Adam(net.parameters(),lr=learning_rate,
                                 weight_decay=weight_decay) # 定义Adam优化器，包含网络信息，学习率，权重衰减（正则化）
    train_labels = train_labels.detach().numpy()
    train_labels = train_labels.reshape(len(train_labels))
    test_labels = test_labels.detach().numpy()
    test_labels = test_labels.reshape(len(test_labels))  # 数据类型转换以便计算rmse和mape
    for epoch in range(num_epochs): # 训练循环，epoch代表训练次数
        for X, Y in train_iter:
            optimizer.zero_grad() # 将模型中各参数的梯度清零。PyTorch的backward()方法计算梯度会默认将本次计算的梯度与缓存中已有的梯度加和
            l = loss(net(X), Y) # 计算损失
            l.backward() # 反向传播 计算各参数对于损失loss的梯度
    ...
    return train_rmse, test_rmse, test_mape  #返回训练误差和验证误差
...
def nn_cv(k, X_train, Y_train, num_epochs, learning_rate, weight_decay, batch_size):
    ...
    for i in range(k):
        ...
        net = get_net(data[0]) # 设置训练集网络
        train_rmse, test_rmse, test_mape = nn_train(net,*data, num_epochs, learning_rate,
                                        weight_decay, batch_size) # 训练
        rmse_train += train_rmse[-1] # 获取训练误差
        rmse_sum += test_rmse[-1] # 获取验证误差
        mape_sum += test_mape[-1]
        if i == 0:
            d2l.plot(list(range(1,num_epochs + 1)),[train_rmse,test_rmse], #第一折验证的训练误差与验证误差随训练次数的变化
